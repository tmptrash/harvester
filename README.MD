...
To run this code, you have to create a local project using the `npm init` command, and then install `js-harvester` and `puppeteer`. For example:
```bash
npm i js-harvester puppeteer-real-browser
```

After that, you can run the file using Node.js:  
```bash
node news
```
...
// The path, where js-harvester is installed
...
// A helper function for loading a specific URL that waits until the page content is fully loaded.
...
// Opens a browser, make it fullscreen and returns current page
async function open() {
